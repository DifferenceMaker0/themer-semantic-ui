    ...
    <div className="grid grid-cols-7 flex-1">
      ...
      {days.map((day) => {
        ...
        return (
          <div
            key={day.toString()}
            className={`border-r border-b border-slate-200 p-2 min-h-[120px] flex flex-col transition-colors hover:bg-slate-50 ${!isSameMonth(day, monthStart) ? 'bg-slate-50/50' : 'bg-white'
              }`}
            onClick={() => onSelectSlot(day)}
          >
            <div
              className={`font-semibold self-start mb-2 ${isToday(day) ? 'bg-blue-600 text-white rounded-full w-7 h-7 flex items-center justify-center' : 'text-slate-700'
                }`}
            >
              {format(day, 'd')}
            </div>
...
export default function CalendarView({ viewMode = 'month', currentDate = new Date(), events = [], projects = [], tasks = [], isLoading = false, onDateSelect, onEventClick }) {
  if (isLoading) {
    return (
  ...
  const renderView = () => {
    switch (viewMode) {
      case 'month':
        return <MonthView {...{ currentDate, events, projects, tasks, onSelectEvent: onEventClick, onSelectSlot: onDateSelect }} />;
      case 'week':
