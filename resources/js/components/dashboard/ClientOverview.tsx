            ...
            recentClients.map((client) => (
              <Link key={client.id} href={`/clients/${client.id}`}>
                <div className="flex items-center justify-between p-4 rounded-lg border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all duration-200 cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${client.client_tier === 'A' ? 'bg-emerald-100' :
                      client.client_tier === 'B' ? 'bg-blue-100' : 'bg-slate-100'
                      }`}>
                      <Building2 className={`w-6 h-6 ${client.client_tier === 'A' ? 'text-emerald-600' :
                  ...
                  <div className="text-right">
                    <Badge className={`${client.client_tier === 'A' ? 'bg-emerald-100 text-emerald-800' :
