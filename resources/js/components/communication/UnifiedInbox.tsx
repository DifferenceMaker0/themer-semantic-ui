...
export default function UnifiedInbox({ messages = [], clients = [], channels = [], projects = [], onMessageUpdate, isLoading = false }) {
            ...
            {isLoading ? (
              ...
              messages.map((message) => {
                ...
                return (
                  <div
                    key={message.id}
                    className={`group flex items-center gap-4 p-4 rounded-lg border transition-all duration-200 hover:border-slate-300 hover:bg-slate-50 cursor-pointer ${message.status === 'unread'
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${message.status === 'unread' ? 'bg-blue-100' : 'bg-slate-100'
                        }`}>
                        <ChannelIcon className={`w-5 h-5 ${message.status === 'unread' ? 'text-blue-600' : 'text-slate-600'
                          }`} />
                      </div>
                      {message.is_flagged && (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`font-medium truncate ${message.status === 'unread' ? 'text-slate-900' : 'text-slate-700'
                          }`}>
                      ...
                      <div className={`text-sm mb-2 ${message.status === 'unread' ? 'text-slate-900' : 'text-slate-600'
                        }`}>
                        {message.subject && (
