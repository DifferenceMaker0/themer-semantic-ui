...
function TwoFactorVerificationStep({ onClose, onBack, }) {
    ...
    return (_jsx(Form, { ...confirm.form(), onSuccess: () => onClose(), resetOnError: true, resetOnSuccess: true, children: ({ processing, errors, }) => (_jsx(_Fragment, { children: _jsxs("div", { ref: pinInputContainerRef, className: "relative w-full space-y-3", children: [_jsxs("div", { className: "flex w-full flex-col items-center space-y-3 py-2", children: [_jsx(InputOTP, { id: "otp", name: "code", maxLength: OTP_MAX_LENGTH, onChange: setCode, disabled: processing, pattern: REGEXP_ONLY_DIGITS, children: _jsx(InputOTPGroup, { children: Array.from({ length: OTP_MAX_LENGTH }, (_, index) => (_jsx(InputOTPSlot, { index: index }, index))) }) }), _jsx(InputError, { message: errors?.confirmTwoFactorAuthentication?.code })] }), _jsxs("div", { className: "flex w-full space-x-5", children: [_jsx(Button, { type: "button", variant: "outline", className: "flex-1", onClick: onBack, disabled: processing, children: "Back" }), _jsx(Button, { type: "submit", className: "flex-1", disabled: processing || code.length < OTP_MAX_LENGTH, children: "Confirm" })] })] }) })) }));
}
export default function TwoFactorSetupModal({ isOpen, onClose, requiresConfirmation, twoFactorEnabled, qrCodeSvg, manualSetupKey, clearSetupData, fetchSetupData, errors, }) {
    ...
    return (_jsx(Dialog, { open: isOpen, onOpenChange: (open) => !open && onClose(), children: _jsxs(DialogContent, { className: "sm:max-w-md", children: [_jsxs(DialogHeader, { className: "flex items-center justify-center", children: [_jsx(GridScanIcon, {}), _jsx(DialogTitle, { children: modalConfig.title }), _jsx(DialogDescription, { className: "text-center", children: modalConfig.description })] }), _jsx("div", { className: "flex flex-col items-center space-y-5", children: showVerificationStep ? (_jsx(TwoFactorVerificationStep, { onClose: onClose, onBack: () => setShowVerificationStep(false) })) : (_jsx(TwoFactorSetupStep, { qrCodeSvg: qrCodeSvg, manualSetupKey: manualSetupKey, buttonText: modalConfig.buttonText, onNextStep: handleModalNextStep, errors: errors })) })] }) }));
}
//# sourceMappingURL=two-factor-setup-modal.js.map